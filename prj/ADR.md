...
(맥: `~/Library/Application Support/pathguard/config.yml`, 윈도우: `%APPDATA%\pathguard\config.yml`).
...
**결정 — 메뉴 구성(위→아래)**:
```
Pathguard              (헤더, 비활성 — 표시 이름, ADR-0011)
...
**맥락**: repo/CLI 이름이 `pathguard`(소문자·하이픈)라 사람이 보는 곳엔 투박하다.

**결정**:
- **표시 이름(사람이 보는 곳) = `Pathguard`**: README 제목, 트레이 메뉴 헤더,
  앱 번들 표시명(macOS `CFBundleName`), 윈도우 실행파일 제품명, About 창, 네이티브 알림 타이틀.
- **식별자(기계가 쓰는 곳) = `pathguard`**: repo 이름, 바이너리/패키지명, 설정 디렉터리
  (`~/Library/Application Support/pathguard`, `%APPDATA%\pathguard`), 번들 ID 요소.

**결과**: 빌드/패키징 단계에서 표시명 메타데이터를 `Pathguard`로 설정(T-0014). 식별자는 그대로.
