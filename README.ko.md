...
사전 빌드 바이너리는 [Releases](../../releases)에 있습니다. 현재 **미서명**이라 macOS·Windows가
첫 실행 시 경고합니다 — 아래 우회 방법 참고. 첫 실행 시 기본 설정 파일을 생성하며(메뉴 **설정…** 으로 열림),
`watch`를 동기화 폴더로 맞춘 뒤 **지금 검사** 하면 아이콘 색으로 결과가 뜨고 초과 파일 클릭 시
Finder/탐색기에서 선택해 줍니다. 메뉴바 에이전트라 **Dock 아이콘 없이** 메뉴막대에 자(ruler) 아이콘으로 뜹니다.

**macOS — Homebrew** ([tap](https://github.com/msjang/homebrew-tap)):
...
1. [Releases](../../releases)에서 `Pathguard-macos-universal.zip`(애플실리콘·인텔 공용)을 받아 압축 해제.
