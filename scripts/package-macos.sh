#!/usr/bin/env bash
# Package the macOS build: a "Sync Pathguard.app" menu-bar agent + the pathguard
...
<plist version="1.0">
<dict>
	<key>CFBundleName</key><string>Sync Pathguard</string>
	<key>CFBundleDisplayName</key><string>Sync Pathguard</string>
	<key>CFBundleIdentifier</key><string>io.github.msjang.sync-pathguard</string>
	<key>CFBundleExecutable</key><string>sync-pathguard</string>
...
echo "Zipping…"
ditto -c -k --keepParent "$APP" "dist/Sync-Pathguard-macos-${ARCH}.zip"
( cd dist && zip -q "pathguard-macos-${ARCH}.zip" pathguard )

echo "→ dist/Sync-Pathguard-macos-${ARCH}.zip"
